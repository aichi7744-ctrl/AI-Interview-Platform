...
const [timeLeft, setTimeLeft] = useState(questions?.[0]?.timeLimit || 60);
    ...
    const runIntro = async () => {
      if (isIntroPhase) {
        await speakText(
          ...
          I hope you're feeling confidnt and ready`,
        );

        await speakText(
          "I'll ask you a few questions. Just annswer naturally, and take your time. Let's begin.",
        );
  ...
  }, [isIntroPhase, currentQuestion?._id]);
  ...
  recognition.onend = () => {
    ...
    // Restart automatically if allowed
...
const toggleMic = () => {
 ...
 const submitAnswer = async () => {
  ...
  try {
    const result = await axios.post(
      ...
      },
      {
        ...
        timeout: 30000, // prevent infinite waiting
      }
    );
    ...
    setFeedback(result.data.feedback || "Answer submitted successfully.");


    speakText(result.data.feedback || "Answer submitted successfully.");
  } catch (error) {
    console.error(
      "Submit error:",
  ...
  return (
    ...
    via-white to-teal-100 flex items-center justify-center
    p-4 sm:p-6"
    >
      <div
        className="w-full max-w-350 min-h-[80vh]
      bg-white rounded-3xl shadow-2xl border border-gray-200
      flex flex-col lg:flex-row overflow-hidden"
      >
        {/*video section*/}
        <div
          className="w-full lg:w-[35%] bg-white
        ...
        >
          ...
          <textarea
            ...
            className="flex-1 bg-gray-100 p-4 sm:p-6 rounded-2xl
               resize-none outline-none border border-gray-50
            ...
            >
              ...
              <button
                ...
                className="w-full bg-gradient-to-r 
                  from-emerald-600 to-teal-500 text-white py-3
                  rounded-xl shadow-md hover:opacity-90 transition
